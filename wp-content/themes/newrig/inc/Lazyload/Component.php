...
class Component implements Component_Interface {
	...
	public function action_customize_register_lazyload( WP_Customize_Manager $wp_customize ) {
		$lazyload_choices = [
			'lazyload'    => __( 'Lazy-load on (default)', 'newrig' ),
			'no-lazyload' => __( 'Lazy-load off', 'newrig' ),
		];
		...
		$wp_customize->add_control(
			'lazy_load_media',
			[
				'label'           => __( 'Lazy-load images', 'newrig' ),
				'section'         => 'theme_options',
				'type'            => 'radio',
				'description'     => __( 'Lazy-loading images means images are loaded only when they are in view. Improves performance, but can result in content jumping around on slower connections.', 'newrig' ),
	...
	public function action_enqueue_lazyload_assets() {
		wp_enqueue_script(
			'newrig-lazy-load-images',
			get_theme_file_uri( '/assets/js/lazyload.min.js' ),
			[],
			newrig()->get_asset_version( get_theme_file_path( '/assets/js/lazyload.min.js' ) ),
			false
		);
		wp_script_add_data( 'newrig-lazy-load-images', 'defer', true );
		wp_script_add_data( 'newrig-lazy-load-images', 'precache', true );
	}
